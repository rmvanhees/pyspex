...
def mask2slice(mask: NDArray[bool]) -> slice | tuple | NDArray[bool] | None:
...
class SCItlm:
    ...
    def append(self: SCItlm, sci: SCItlm) -> None:
        ...
        if isinstance(self.images, tuple) and self.images:
            self.images = self.images[0]

        if isinstance(sci.images, tuple):
            sci.images = sci.images[0]
