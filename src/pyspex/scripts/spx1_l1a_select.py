...
# --------------------------------------------------
def inv_sec_of_day(
    reference_day: datetime, sec_of_day: np.ndarray, epoch: datetime = DEFAULT_EPOCH
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Copy selected data from one SPEXone L1A product"
            " into a new SPEXone L1A product"
        )
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="be verbose, default be silent"
    )
    parser.add_argument("--mps_id", type=int, default=None, help="select on MPS-ID")
    ...
    parser.add_argument(
        ...
        default=".",
        help=(
            "name of directory to store the new Level-1A"
            " product, default: current working directory"
        ),
    )
    parser.add_argument(
        "l1a_product", default=None, help="name of SPEXone Level-1A product"
    )
    ...
    if args.mps_id is not None:
        ...
        if name.find("PACE_SPEXone_CAL") != -1:
            name = name.replace(
                "PACE_SPEXone_CAL", f"PACE_SPEXone_CAL_mps{args.mps_id:03d}"
            )
        else:
            name = name.replace("PACE_SPEXone", f"PACE_SPEXone_mps{args.mps_id:03d}")
