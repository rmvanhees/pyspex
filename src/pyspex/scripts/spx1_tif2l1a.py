...
def get_stimulus(hdr: np.ndarray) -> xr.Dataset:
    ...
    xr_wv = xr.DataArray(
        ds_dict["Wavelength (nm)"],
        coords=[ds_dict["Wavelength (nm)"]],
        dims=["wavelength"],
        attrs={"long_name": "wavelength of stimulus", "units": "nm"},
    )
    ...
    xr_sign = xr.DataArray(
        ...
        dims=["wavelength"],
        coords=[ds_dict["Wavelength (nm)"]],
        attrs={"long_name": "signal of stimulus", "units": "W.m-2.sr-1.um-1"},
    )
...
def main() -> None:
    ...
    if args.inp_tif:
        ...
        if args.lineskip:
            n_samples = int(hdr["Enabled lines"]) * int(hdr["Column dimension"])
        else:
            n_samples = int(hdr["Row dimension"]) * int(hdr["Column dimension"])
    ...
    if n_samples == 0x400000:
        ...
        if args.lineskip:
            table_id = get_table_id("/nfs/SPEXone/share/ckd", hdr["Line-enable-array"])
        else:
            table_id = get_table_id(
                "/nfs/SPEXone/share/ckd", hdr["Flexible binning table"]
            )
    ...
    sci_hk["DET_EXPTIME"] = int(1e7 * float(hdr["Exposure time (s)"]) / 129 - 0.43 * 20)
    ...
    with LV1gse(prod_name) as gse:
        ...
        if "Detector illumination" in hdr:
            ...
            gse.set_attr("Illumination_level", float(hdr["Detect. illumination e/ms"]))
        if "Detector temperature (K)" in hdr:
            gse.set_attr("Detector_temperature", float(hdr["Detector temperature (K)"]))
        if "Detector temperature (K)" in hdr:
            gse.set_attr("Optics_temperature", float(hdr["Optics temperature (K)"]))
        ...
        if "Illuminated field ACT" in hdr:
            gse.write_attr_act(
                float(hdr["Rotation stage ACT angle"]),
        ...
        if "Illuminated field ALT" in hdr:
            gse.write_attr_alt(
                float(hdr["Rotation stage ALT angle"]),
                float(hdr["Illuminated field ALT"]),
            )
        else:
            gse.write_attr_alt(float(hdr["Rotation stage ALT angle"]))
        if "DoLP input" in hdr:
            gse.write_attr_polarization(
                float(hdr["AoLP input (deg)"]), float(hdr["DoLP input"])
            )
        if "Spectral data stimulus" in hdr and "Illuminated viewing angle" in hdr:
