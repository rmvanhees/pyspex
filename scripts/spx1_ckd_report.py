...
# - local functions --------------------------------
def init_plot_file(key: str, ckd: CKDio, ckd_ref: CKDio | None = None) -> tuple:
    """Initialize CKD report."""
    fig_info_in = FIGinfo()
    fig_info_in.add(
        "processor_version", (ckd.processor_version, ckd.git_commit), fmt="{} ({})"
    )
    ...
    if ckd_ref is not None:
        fig_info_in.add(
            "reference_version",
            (ckd_ref.processor_version, ckd_ref.git_commit),
            fmt="{} ({})",
        )
...
def add_dark_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    if ckd_version == "v1":
        plot.draw_signal(
            dark_ckd["dark_offset"], title="dark offset", fig_info=fig_info_in.copy()
        )
        plot.draw_hist(
            dark_ckd["dark_offset"],
            bins=161,
            vrange=[-5.5, 2.5],
            title="dark offset",
        ...
        plot.draw_signal(
            dark_ckd["offset_long"], title="offset (long)", fig_info=fig_info_in.copy()
        )
        plot.draw_hist(
            dark_ckd["offset_long"],
            bins=161,
            vrange=[-5.5, 2.5],
            title="offset (long)",
            fig_info=fig_info_in.copy(),
    ...
    plot.draw_signal(
        dark_ckd["dark_current"], title="dark current", fig_info=fig_info_in.copy()
    )
    plot.draw_hist(
        dark_ckd["dark_current"],
        bins=101,
        vrange=[1.5, 6.5],
        title="dark current",
        fig_info=fig_info_in.copy(),
    ...
    if ckd_version == "v1":
        if ref_version == "v1":
            plot.draw_signal(
                dark_ckd["dark_offset"] - ref_ckd["dark_offset"],
            ...
            plot.draw_signal(
                dark_ckd["dark_offset"] - ref_ckd["offset_long"],
                title="dark offset - reference",
                fig_info=fig_info_in.copy(),
                zscale="diff",
            )
        ...
        if ref_version == "v1":
            plot.draw_signal(
                dark_ckd["offset_long"] - ref_ckd["dark_offset"],
            ...
            plot.draw_signal(
                dark_ckd["offset_short"] - ref_ckd["offset_short"],
                title="offset (short) - reference",
                fig_info=fig_info_in.copy(),
                zscale="diff",
            )
            plot.draw_signal(
                dark_ckd["offset_long"] - ref_ckd["offset_long"],
                title="offset (long) - reference",
                fig_info=fig_info_in.copy(),
                zscale="diff",
            )
    ...
    plot.draw_signal(
        dark_ckd["dark_current"] - ref_ckd["dark_current"],
...
def add_noise_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    plot.draw_signal(
        1 / noise_ckd["g"], vrange=[11.5, 15], title=g_str, fig_info=fig_info_in.copy()
    )
    plot.draw_hist(
        1 / noise_ckd["g"],
        bins=161,
    ...
    n_str = "read noise"
    plot.draw_signal(noise_ckd["n"], title=n_str, fig_info=fig_info_in.copy())
    plot.draw_hist(
        noise_ckd["n"],
        bins=161,
        vrange=[0, 8],
    ...
    if ref_ckd is not None:
        plot.draw_signal(
            noise_ckd["g"] - ref_ckd["g"],
            title=g_str + " - reference",
            fig_info=fig_info_in.copy(),
            zscale="diff",
        )
        plot.draw_signal(
            noise_ckd["n"] - ref_ckd["n"],
...
def add_nlin_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    if "c" in nlin_ckd:
        plot.draw_signal(
            nlin_ckd["f1"], title="parameter f1", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["f2"], title="parameter f2", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["c"], title="sigmoid parameter c", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["r0"], title="sigmoid parameter r0", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["r1"], title="sigmoid parameter r1", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["r2"], title="sigmoid parameter r2", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["r3"], title="sigmoid parameter r3", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["r4"], title="sigmoid parameter r4", fig_info=fig_info_in.copy()
        )
        plot.draw_signal(
            nlin_ckd["m0"], title="sigmoid parameter m0", fig_info=fig_info_in.copy()
        )
...
def add_prnu_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    plot.draw_signal(prnu_ckd, fig_info=fig_info_in.copy(), title=prnu_str)
    if ref_ckd is not None:
        prnu_str = "Pixel Response Non-Uniformity (reference)"
        plot.draw_signal(ref_ckd, fig_info=fig_info_in.copy(), title=prnu_str)
    plot.draw_hist(
        prnu_ckd,
        bins=201,
        vrange=[0.95, 1.05],
...
def add_wave_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    plot.draw_signal(
        wave_ckd["wave_full"][0, ...], fig_info=fig_info_in.copy(), title=wave_s_str
    )
    wave_p_str = "wavelength grid of the P spectra"
    plot.draw_signal(
        wave_ckd["wave_full"][1, ...], fig_info=fig_info_in.copy(), title=wave_p_str
    )
    wave_str = "common wavelength grid for S and P"
    plot.draw_signal(
        wave_ckd["wave_common"], fig_info=fig_info_in.copy(), title=wave_str
    )

    ref_ckd = ckd_ref.wavelength() if ckd_ref is not None else None
    if ref_ckd is not None:
        plot.draw_signal(
            wave_ckd["wave_common"] - ref_ckd["wave_common"],
            title=wave_str + " - reference",
            fig_info=fig_info_in.copy(),
            zscale="diff",
        )
        plot.draw_signal(
            wave_ckd["wave_full"][0, ...] - ref_ckd["wave_full"][0, ...],
            title=wave_s_str + " - reference",
            fig_info=fig_info_in.copy(),
            zscale="diff",
        )
        plot.draw_signal(
            wave_ckd["wave_full"][1, ...] - ref_ckd["wave_full"][1, ...],
...
def add_rad_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    plot.draw_signal(rad_ckd[:, 0, :], fig_info=fig_info_in.copy(), title=rad_s_str)
    rad_p_str = rad_ckd.attrs["long_name"] + " (P)"
    plot.draw_signal(rad_ckd[:, 1, :], fig_info=fig_info_in.copy(), title=rad_p_str)
...
def add_pol_figs(ckd: CKDio, ckd_ref: CKDio | None = None) -> None:
    ...
    plot.draw_signal(pol_ckd["pol_m_q"], fig_info=fig_info_in.copy(), title=pol_q_str)
    pol_u_str = pol_ckd["pol_m_u"].attrs["long_name"]
    plot.draw_signal(pol_ckd["pol_m_u"], fig_info=fig_info_in.copy(), title=pol_u_str)
    pol_t_str = pol_ckd["pol_m_t"].attrs["long_name"]
    plot.draw_signal(pol_ckd["pol_m_t"], fig_info=fig_info_in.copy(), title=pol_t_str)

    ref_ckd = ckd_ref.polarimetric() if ckd_ref is not None else None
    if ref_ckd is not None:
        plot.draw_signal(
            pol_ckd["pol_m_q"] - ref_ckd["pol_m_q"],
            title=pol_q_str + " - reference",
            fig_info=fig_info_in.copy(),
            zscale="diff",
        )
        plot.draw_signal(
            pol_ckd["pol_m_u"] - ref_ckd["pol_m_u"],
            title=pol_u_str + " - reference",
            fig_info=fig_info_in.copy(),
            zscale="diff",
        )
        plot.draw_signal(
            pol_ckd["pol_m_t"] - ref_ckd["pol_m_t"],
...
def main() -> None:
    ...
    parser.add_argument(
        "--ref_ckd_file", default=None, help="name of reference CKD product"
    )
